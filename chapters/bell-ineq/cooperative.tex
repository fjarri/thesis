...
We sample this distribution by transforming it to a form where we could use the well-known von Neumann rejection algorithm.
...
The Gaussian parts can be sampled exactly using conventional methods, while the distribution core requires requires the application of rejection sampling.
...
\begin{figure}
    ...
    The filled area corresponds to the estimated sampling error around the mean of $\Delta(\theta)$ for the sampled state, while the dashed line is the exact quantum mechanical prediction of this value.}
