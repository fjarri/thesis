...
\label{eqn:bell-ineq:cooperative:G}
    ...
    = \langle N \vert
        ...
        \vert N \rangle.
\end{eqn}
They are proportional (exactly for $I=J=N$ and in the first approximation for other variants) to the probability of observing $I$ ``$+$''-polarized photons at detector $1$ and $J$ ``$-$''-polarized photons at detector $2$.
The linear combination parameter depends on the relative polarizer angle $\theta$ as $\gamma = \cos^2 \theta$, and auxiliary functions are introduced so that:
\begin{eqn}
    ...
    ={} & :\left(
        ...
        \right)^{J}: .
\end{eqn}
The ``$\infty$'' case and the corresponding $G^{IJ}(\infty,N)$ correlation stand for the same measurement as for $G^{IJ}(\gamma,N)$, but with no polarizer in the second detector.
