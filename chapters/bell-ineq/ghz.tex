...
\begin{figure}
    ...
    The dotted reference line $\log_2 \mathrm{error} \propto M$ shows the point at which the sampling errors would give scaling properties as slow as an experimental measurement.}%endcaption
...
Bell violations were verified in all cases, while genuine multipartite violations of \abbrev{lhv} requiring all $M$ observers to participate, with $F/F_{QM}>1/\sqrt{2}$, were verified for $M<50$.

To understand the source of sampling errors, we investigated the scaling of errors with system-size for single measurements of a low-order spin correlation (\figref{bell-ineq:ghz:violation},~(b)), in addition to the exponentially large numbers of measurements in the expectation values $F$.
