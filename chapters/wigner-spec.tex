...
This chapter contains several theorems describing transformations of specific operator sequences, encountered in master equations describing \abbrev{bec} evolution.
We will use these theorems in \charef{wigner-bec} to obtain a correspondence between operator evolution equations and functional derivatives of a classical field $\Psi$, which act on the Wigner functional instead of the quantum density matrix.

In order to give an idea about the form of expressions we are targeting, we will outline the master equation of interest.
It will be described in more detail in \secref{wigner-bec:hamiltonian} and \secref{wigner-bec:master-eqn}.
We are interested in transforming a master equation of the form
\begin{eqn}
...
\end{eqn}
where $\hat{H}$ is the Hamiltonian of the system, and the $\mathcal{L}$ are nonlinear Liouville loss terms
\begin{eqn}
\label{eqn:wigner-spec:loss-op}
...
\end{eqn}
Here $\lvec = (l_1,\,\ldots,\,l_C)^T$ is the identifier of a loss process, with $l_j$ denoting the number of particles of the component $j$ lost in the event, and
\begin{eqn}
    \hat{O}_{\lvec}
    \equiv \hat{O}_{\lvec} (\Psiopvec)
    = \prod_{j=1}^C \Psiop_j^{l_j} (\xvec).
\end{eqn}
The Hamiltonian, in turn, has the general form
\begin{eqn}
\label{eqn:wigner-spec:H}
    \hat{H} = \int \upd \xvec \sum_{j=1}^C \sum_{k=1}^C \left\{
        ...
        + \frac{1}{2} \int \upd \xvec^\prime
            ...
            \Psiop_j \Psiop_k^\prime
    \right\},
\end{eqn}
where $K_{jk} = \alpha \nabla^2 + \beta(\xvec, t)$ is a linear operator that does not depend on $\Psiop_j$, possibly containing a linear occurrence of a Laplacian.
...
The simplest non-trivial element of the Hamiltonian~\eqnref{wigner-spec:H} is the second-order operator $\Psiop_j^\dagger \Psiop_k$, which may describe, for instance, a potential or a coupling field.

\begin{theorem}
\label{thm:wigner-spec:w-commutator1}
    For a Hilbert-Schmidt operator $\hat{A}$ with a corresponding Wigner functional $\mathcal{W}[\hat{A}] \equiv (\mathcal{W}[\hat{A}])[\bPsi]$, 
    \begin{eqn*}
...
Commutators with a Laplacian inside require special treatment, because the Laplacian acts on basis functions and, in general, cannot be treated like a constant.
For our purposes, we only need the case where it occurs linearly in a Hamiltonian (referred to as the kinetic term).
...
\end{theorem}
\begin{proof}
First, it is obvious from the linearity of the Wigner transformation that an integral or a derivative acting on coordinates can be moved in and out of the transformation:
\begin{eqn}
...
The last of the terms in the Hamiltonian~\eqnref{wigner-spec:H} is the fourth order nonlinear interaction term.
...
\label{thm:wigner-spec:w-commutator2}
    ...
    \begin{eqn*}
        ...
        \right] \\
        & = \iint \upd\xvec\, \upd\xvec^\prime \left(
...
In this section we will derive the theorem for transformation of the loss terms~\eqnref{wigner-spec:loss-op}.
...
\label{lmm:wigner-spec:swap-differential}
    For a function $\Psi \in \mathbb{F}_{\restbasis}$ and a functional operator $\mathcal{F} \in \mathbb{F}_{\restbasis} \rightarrow \mathbb{F}$, and non-negative integers $a$, $b$
    \begin{eqn*}
...
\label{thm:wigner-spec:w-losses}
    The Wigner transformation of the loss operator $\mathcal{L}_{\lvec}$ of form~\eqnref{wigner-spec:loss-op} is
    \begin{eqn*}
...
This theorem is the most complex result needed in this thesis.
It is essential for the treatment of nonlinear loss processes in a \abbrev{bec} using a master equation.
In the next chapters we will use these results to analyse realistic experiments, that often do involve nonlinear loss processes.
