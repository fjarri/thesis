...
The test is essentially the same integration that was used to plot \figref{bec-noise:visibility:ramsey-visibility}, only executed for a shorter time ($t=0.1\un{s}$) for convenience.
...
\item the weak convergence in the total spin $\langle \hat{S}^2_z \rangle$~\eqnref{bec-squeezing:thoery:spin-squares} (fourth order correlation).
\end{itemize}
...
\begin{figure}
    ...
    \includegraphics{figures_generated/test/convergence_gpe_SS-CD.pdf}%

    \caption[Convergence tests, cGPEs]{
    Convergence tests for classical cGPEs.}%endcaption

    \label{fig:numerical:convergence-gpe}
\end{figure}

As a start, we executed our tests for classical \abbrev{cgpe}s to check the order of convergence in deterministic part of the equation.
...
\begin{figure}
    ...
    \includegraphics{figures_generated/test/convergence_wigner_SS-CD.pdf}%

    \caption[Convergence tests, Wigner representation]{
    Convergence tests for the Wigner representation.}%endcaption

    \label{fig:numerical:convergence-wigner}
\end{figure}

The tests for the Wigner representation, plotted in \figref{numerical:convergence-wigner}, which now include the stochastic terms, show a different picture.
...
\begin{figure}
    ...
    Convergence comparison.}%endcaption
