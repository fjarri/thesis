...
Multimode field of a $C$-component \abbrev{bec} in $D$ effective dimensions is described by field operators $\Psiopf_j^{\dagger}(\xvec)$ and $\Psiopf_j(\xvec)$, where $\Psiopf_j^{\dagger}$ creates a bosonic atom of spin $j$, $j = 1 \ldots C$ at a location defined by a $D$-component coordinate vector $\xvec$, and $\Psiopf_j$ destroys one.
