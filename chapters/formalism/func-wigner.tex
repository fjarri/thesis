...
\begin{split}
	\hat{D}[\Lambda, \Lambda^*]
	& = \exp \int d\xvec \Psiop(\xvec)^\dagger \Lambda(\xvec)
		\exp \left( -\int d\xvec \Psiop(\xvec) \Lambda^*(\xvec) \right)
		\exp \frac{1}{2} \left[
			\int d\xvec^\prime \Psiop(\xvec^\prime)^\dagger \Lambda(\xvec^\prime),
			\int d\xvec \Psiop(\xvec) \Lambda^*(\xvec)
		\right] \\
	& = \exp \int d\xvec \Psiop^\dagger \Lambda
		\exp \left( -\int d\xvec \Psiop \Lambda^* \right)
		\exp \left(
