...
The Wigner function always exists for any density matrix, which makes the correspondence $W \leftrightarrow \hat{\rho}$ a bijection.
...
\end{split}
\end{equation*}
where we used \lmmref{formalism:c-numbers:zero-integrals-2} to evaluate integral over $\lambda$.
\end{proof}
...
If $\mathcal{W} [ \hat{\rho} ] = W(\alpha, \alpha^*)$, then
\begin{equation*}
\begin{split}
	\mathcal{W} [ \hat{a} \hat{\rho} ]
		& = \left( \alpha + \frac{1}{2} \frac{\partial}{\partial \alpha^*} \right) W,
	\quad
	\mathcal{W} [ \hat{a}^\dagger \hat{\rho} ]
		= \left( \alpha^* - \frac{1}{2} \frac{\partial}{\partial \alpha} \right) W, \\
	\mathcal{W} [ \hat{\rho} \hat{a} ]
		& = \left( \alpha - \frac{1}{2} \frac{\partial}{\partial \alpha^*} \right) W,
	\quad
	\mathcal{W} [ \hat{\rho} \hat{a}^\dagger ]
		= \left( \alpha^* + \frac{1}{2} \frac{\partial}{\partial \alpha} \right) W.
\end{split}
\end{equation*}
\end{theorem}
