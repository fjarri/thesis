...
\end{split}
\end{equation*}
Using \lmmref{formalism:transformations:swap-differential} to swap $\Psi_c$ and $\delta / \delta \Psi_c$:
\begin{equation*}
\begin{split}
	= & \prod_{c=1}^C \left(
			\sum_{j=0}^{l_c}
...
\[
	...
	(\Psi_c^*)^{l_c - k}.
\]
Using \lmmref{formalism:transformations:sum-rearrangement}:
\begin{equation*}
\begin{split}
	= & \prod_{c=1}^C \left(
			\sum_{j=0}^{l_c}
...
\todo{Here's a good place for ``multiple application'' operator.}
\begin{equation*}
\begin{split}
