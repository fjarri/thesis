...
Single-mode \defref{wigner:sm:w-transformation} of Wigner transformation can be extended to the case of many modes.
Mode operators $\hat{a}_\nvec$, $\nvec \in \restbasis$, obey bosonic commutation relations:
\begin{eqn}
\label{eqn:wigner:mm:commutators}
...
It can be proved, same as in \thmref{wigner:sm:w-real} for the single-mode case, that $W[\hat{A}]$ is real if $\hat{A}$ is Hermitian.
...
\begin{theorem}[Multimode extension of \thmref{wigner:sm:correspondences}]
...
\end{theorem}
\begin{proof}
The procedure is the same as in \thmref{wigner:sm:correspondences}.
\end{proof}

\begin{lemma}[Multimode extension of \lmmref{wigner:sm:moments-from-chi}]
...
\begin{eqn}
	...
	& = \left( \prod_{\nvec \in \restbasis} \sum_{j_{\nvec}, k_{\nvec}} \right)
		...
		\rangle.
\end{eqn}
Same as in \lmmref{wigner:sm:moments-from-chi}, differentiating the resulting expression and setting $\blambda = 0$ will leave only term with required $j_{\nvec} = r_{\nvec}$ and $k_{\nvec} = s_{\nvec}$.
\end{proof}
...
\begin{theorem}[Multimode extension of \thmref{wigner:sm:moments}]
...
\end{theorem}
\begin{proof}
The proof is carried out similarly to \thmref{wigner:sm:moments}: integrals over $\alpha_{\nvec}$ are eliminated one by one using \lmmref{c-numbers:fourier-of-moments}, resulting integrals over $\lambda_{\nvec}$ are dealt with using integration by parts and \lmmref{c-numbers:zero-delta-integrals}, until we get the right part of \lmmref{wigner:mm:moments-from-chi}.
\end{proof}
