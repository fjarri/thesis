...
Single-mode \defref{mm-wigner:sm:w-transformation} of Wigner transformation can be extended to a case of many modes.
...
\label{lmm:mm-wigner:mm:moments-from-chi}
	For a system with a density matrix $\rho$ and the corresponding characteristic function $\chi_W$:
	\begin{eqn*}
...
\begin{eqn}
	...
	& = \left( \prod_{\nvec \in \restbasis} \sum_{j_{\nvec}, k_{\nvec}} \right)
		...
		\right\rangle.
\end{eqn}
Same as in \lmmref{mm-wigner:sm:moments-from-chi}, differentiating the resulting expression and setting $\blambda = 0$ will leave only the term with required $j_{\nvec} = r_{\nvec}$ and $k_{\nvec} = s_{\nvec}$.
\end{proof}
...
\label{thm:mm-wigner:mm:moments}
	For a system with a density matrix $\rho$ and the corresponding Wigner function $W(\balpha)$, and any non-negative integer $r$, $s$:
	\begin{eqn*}
		\left\langle \symprod{
			\prod_{\nvec \in \restbasis}
			\hat{a}_{\nvec}^{r_{\nvec}} (\hat{a}_{\nvec}^\dagger)^{s_{\nvec}}
