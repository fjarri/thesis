...
\end{center}
\caption{Shape of restricted delta function $\delta_P(z - z^\prime)$ with and without explicit cutoff.}
...
\label{eqn:wigner:restricted-delta}
	...
	= \sum\limits_{\nvec \in L} \phi_{\nvec}^* (\xvec) \phi_{\nvec} (\xvec^\prime).
\end{equation}
Shape of restricted delta function for two different grids and two different cutoff is shown in \figref{wigner:restricted-delta}.
