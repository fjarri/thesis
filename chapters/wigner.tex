...
\todo{Calculate the part from unitary evolution.}
...
\end{split}
\end{equation*}
In the first transformation we expanded $\mathcal{L}$ operator.
In the second and third transformations we used the fact that trace is invariant under cyclic permutations to re-order the operators in terms.
In the fourth transformation we reduced two equal terms and recognised the commutator.
Fifth transformation uses the relation~\eqnref{multimode-formalism:functional-commutators} for function commutator.
