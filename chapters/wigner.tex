...
Shape of the module of restricted delta function for two different grids and two different cutoff is shown in \figref{wigner:restricted-delta}.
...
\[
	P [f(\xvec)] = \int d\xvec^\prime \delta_P^*(\xvec - \xvec^\prime) f(\xvec^\prime).
\]
The Hermitian conjugate of $P$ is thus defined as
\[
	(P [f(\xvec)])^\dagger
	= \int d\xvec^\prime \delta_P(\xvec - \xvec^\prime) f^\dagger(\xvec^\prime)
