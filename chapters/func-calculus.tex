...
\begin{eqn}
	\int \fdelta^2 \Lambda \mathcal{F}[\Lambda] \Delta_{\restbasis}[\Lambda]
	& = \left(
			\prod_{\nvec \in \restbasis} \int \upd^2\lambda_{\nvec}
		\right)
		\mathcal{F}(\blambda)
		\prod_{\nvec \in \restbasis} \delta(\Real \lambda_{\nvec}) \delta(\Imag \lambda_{\nvec}) \\
...
\label{lmm:func-calculus:fourier-of-moments}
	...
	\begin{eqn*}
		\int \fdelta^2\Psi\, \Psi^r (\Psi^*)^s \exp
			\int \upd\xvec \left( -\Lambda \Psi^* + \Lambda^* \Psi \right)
		= \pi^{2|\restbasis|}
