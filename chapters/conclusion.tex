...
We then applied this framework to the exact operator equation governing the dynamics of a Bose-Einstein condensate (\abbrev{bec}) and showed how to transform it into the equivalent partial differential equation (\abbrev{pde}).
An approximation (Wigner truncation) was introduced, which allowed to simplify this equation further, and turn it into a set of stochastic differential equations (\abbrev{sde}s) for trajectories in the phase space.
...
The latter is a huge advantage in the modern word of multi-core and multi-node computations and the recent advent of general calculations on graphical processing units (\abbrev{gpu}s).
In particular, the use of modern \abbrev{gpu}s allowed us to simulate the dynamics of hundreds of thousands atoms with thousands of modes with high degree of accuracy on a desktop in the order of hours.
...
In the last chapter we considered a more broad topic of the ability of phase space methods to simulate quantum mechanical systems, and their differences from local hidden variable (\abbrev{lhv}) theories.
We showed that, due to weaker restrictions on the codomains of the moments that correspond to physical observables, such methods are indeed capable of demonstrating such essential \abbrev{qm} properties as the violation of Bell inequalities.
We were able to calculate the correlations involving every particle in the system in highly entangled Greenberger-Horne-Zeilinger-Mermin (\abbrev{ghzm}) states for up to 60 particles.
