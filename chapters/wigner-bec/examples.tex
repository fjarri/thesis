...
We will first consider a simple case of a single-component \abbrev{bec} with a 3-body loss process and no unitary evolution (the same as described by Norrie \textit{et~al}~\cite{Norrie2006a}).
For this system, we have $K \equiv 0$, $U \equiv 0$ and $\hat{O} = \Psiop^3$ (and, consequently, $O = \Psi^3$), and we also denote $\gamma = 6\kappa$ ($\gamma$ and $\kappa$ represent ``theoretical'' and ``experimental'' loss rates, respectively; see~\eqnref{bec-noise:wigner:loss-rates} and the surrounding paragraph).
