...
Note the asymmetry in the expression for Stratonovich term: if $B = B(\alpha)$, then $\bm{s} \equiv 0$.
...
\begin{theorem}
	...
	\end{eqn*}
	\todo{Here we have a conjugated term in the SDE as opposed to previous theorems, because the order of indices in the diffusion term of FPE is different (j-k-j-k) instead of (j-k-k-j).
	It was made so to correspond to the ``real'' FPE for BEC.
	Perhaps it would be better to make $\mathcal{B}$ in SDEs be non-conjugated here and instead say that $\mathcal{B} = \delta / \delta \Psi_j^* \hat{O}^*$ when we talk about BEC.}
\end{theorem}
\begin{proof}
Considering $\Psi_j = \sum_{\nvec \in \restbasis_j} \phi_{j,\nvec} \alpha_{j,\nvec}$ and replacing functional derivatives with ordinary ones:
\begin{eqn}
...
\begin{eqn}
	\int d\xvec \phi_{j,\mvec} \phi_{k,\nvec}^* \sum_{\lvec} \mathcal{B}_{\lvec}^{(j)} (\mathcal{B}_{\lvec}^{(k)})^*
