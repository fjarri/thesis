...
we will transform it to ordinary differential equation using Wigner transformation from \defref{wigner:mc:w-transformation}.

Namely, the term with $K_j$ is transformed using \thmref{wigner-spec:w-commutator1} and \thmref{wigner-spec:w-laplacian-commutator1} (since $K_j$ is basically a sum of Laplacian operator and functions of $\xvec$):
\begin{eqn}
...
Nonlinear term is transformed with \thmref{wigner-spec:w-commutator2} (minding the locality of interaction and integrating over $\xvec$):
\begin{eqn}
	\mathcal{W} \left[
		[
	...
	& = \int d\xvec \frac{U_{jk}}{2} \left(
		\frac{\delta}{\delta \Psi_j} \left(
			- \Psi_j \Psi_k \Psi_k^*
			+ \frac{\delta_{\restbasis_j}(\xvec, \xvec)}{2} ( \delta_{jk} \Psi_k + \Psi_j )
		\right) \right. \\
	...
	&	\left. + \frac{\delta}{\delta \Psi_k} \left(
			- \Psi_j \Psi_j^* \Psi_k
			+ \frac{\delta_{\restbasis_k}(\xvec, \xvec)}{2} ( \delta_{jk} \Psi_j + \Psi_k )
		\right) \right. \\
	&	\left. + \frac{\delta}{\delta \Psi_k^*} \left(
			\Psi_j \Psi_j^* \Psi_k^*
			- \frac{\delta_{\restbasis_k}(\xvec, \xvec)}{2} ( \delta_{jk} \Psi_j^* + \Psi_k^* )
...
Loss operator is transformed with \thmref{wigner-spec:w-losses}.
...
For now I am assuming that we can drop third- and higher-order derivatives and consider $\delta_{\restbasis_j}(\xvec, \xvec) \ll | \Psi_j |^2$ for any $j$.}
