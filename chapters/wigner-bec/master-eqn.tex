...
\label{eqn:wigner-bec:master-eqn:master-eqn}
	\frac{\upd\hat{\rho}}{\upd t} =
		...
		+ \sum_{\lvec} \kappa_{\lvec} \int \upd \xvec
...
\begin{eqn}
	...
	+ \sum_{\lvec} \kappa_{\lvec} \int \upd \xvec^\prime
		\Trace{
...
In addition, the transformations are based on commutation relations proved in \lmmref{wigner:op-calculus:functional-commutators}.
...
\begin{eqn}
	...
	={} & -2\sum_{\lvec} \kappa_{\lvec}
			\langle
				\Psiop_j^\dagger
				\mathcal{P}_{\restbasis_j} \left[
					\frac{\partial \hat{O}_{\lvec}^\dagger}{\partial \Psiop_j^\dagger}
					\hat{O}_{\lvec}
				\right]
			\rangle.
\end{eqn}
