...
\todo{reference an appendix with mean-field approximation?}

If we impose an energy cutoff $\ecut$ and only take into account low-energy modes of the field,
the general scattering potential $U_{jk}(\xvec - \xvec^\prime)$ can be replaced by the contact potential $U_{jk} \delta(\xvec - \xvec^\prime)$~\cite{Morgan2000} \todo{just how low should the energy be? need to elaborate on that}, giving the effective Hamiltonian
\begin{eqn}
