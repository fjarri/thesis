...
It can be shown (see \appref{fpe-sde} for details) that this truncated equation has a positive-definite diffusion matrix $\mathcal{D}$ and is, therefore, an \abbrev{fpe}, with its solution $W(t)$ being a probability distribution (provided that $W(0)$ was a probability distribution).
This differs from the original Wigner function(al), which can be negative, and is the result of the truncation.
Thus, the solution of this equation may not be equivalent to the solution of the original master equation~\eqnref{wigner-bec:master-eqn:master-eqn} (if the corresponding density matrices have non-positive Wigner functions), but if the truncation condition is satisfied, the difference is small (see \charef{exact} for a simple comparison, and references therein).
...
The equation can be further transformed to an equivalent set of \abbrev{sde}s using \thmref{fpe-sde:corr:fpe-sde-func} with
\begin{eqn}
	\mathcal{B}_{j\lvec}
...
The required expectations of symmetrically ordered operator products can be obtained by averaging results from multiple independent integration trajectories according to \thmref{wigner:mc:moments} since the truncated $W$ function is a probability distribution:
\begin{eqn}
