...
\label{lmm:fpe-sde:ito-formula:ito-f-real}
	...
	For the set of \abbrev{sde}s in It\^{o} form
	\begin{eqn*}
...
\label{thm:fpe-sde:ito-formula:mc-ito-f}
	...
	For the \abbrev{sde} in It\^{o} form
	\begin{eqn*}
...
\label{thm:fpe-sde:ito-formula:func-ito-f}
	Given the set of functional \abbrev{sde}s in It\^{o} form
	\begin{eqn*}
		\upd\Psi^{(j)} = \mathcal{A}^{(j)} dt + \sum_{\lvec \in \mathbb{L}} \mathcal{B}_{\lvec}^{(j)} \upd Q_{\lvec},
	\end{eqn*}
	the differential of a functional operator $\mathcal{F}[\Psivec]$ is
	\begin{eqn*}
		\upd \mathcal{F}[\Psivec]
		={} & \int \upd\xvec^\prime \left(
			2 \sum_{j=1}^C \Real \left(
				\mathcal{A}_j^\prime \frac{\fdelta}{\fdelta \Psi_j^\prime}
			\right) \mathcal{F}[\Psivec] \upd t \right. \\
		& + \sum_{j=1}^C \sum_{k=1}^C \sum_{\lvec \in \mathbb{L}}
				\mathcal{B}_{j,\lvec}^\prime
				\mathcal{B}_{k,\lvec}^{\prime *}
				\frac{\fdelta}{\fdelta \Psi_j^\prime}
				\frac{\fdelta}{\fdelta \Psi_k^{\prime *}} \mathcal{F}[\Psivec] \upd t \\
		& \left. + 2 \sum_{j=1}^C \sum_{\lvec \in \mathbb{L}}
			\Real \left(
				\mathcal{B}_{j,\lvec}^\prime
				\upd Q_{\lvec}^\prime
				\frac{\fdelta}{\fdelta \Psi_j^\prime}
		...
		\right).
	\end{eqn*}
	Here $\Psi_j$, $\mathcal{A}_j$, $\mathcal{B}_{j,\lvec}$, $W$, $\mathbb{L}$, and $Q_{\lvec}$ are defined the same as in \thmref{fpe-sde:corr:fpe-sde-func}.
\end{theorem}
\begin{proof}
...
\end{eqn}
Now, treating the functional operator as a function of $M$ complex vectors
\begin{eqn}
	\mathcal{F} \equiv \mathcal{F}[\mathcal{C}_{\restbasis_1}(\balpha^{(1)}), \ldots, \mathcal{C}_{\restbasis_C}(\balpha^{(C)})],
\end{eqn}
we can use \thmref{fpe-sde:ito-formula:mc-ito-f} with drift vectors
\begin{eqn}
...
\begin{eqn}
	B_{\mvec,(\pvec,\lvec)}^{(j)}
	= \int \upd\xvec\, \phi_{j,\mvec}^* \mathcal{B}_{\lvec}^{(j)} \phi_{\pvec}.
\end{eqn}
...
\begin{eqn}
	...
	& + \sum_{j=1}^C \sum_{k=1}^C
			...
			\int \upd\xvec^\prime \phi_{j,\mvec}^{\prime *} \mathcal{B}_{j,\lvec}^\prime \phi_{\pvec}^\prime
			\int \upd\xvec^{\prime\prime} \phi_{k,\nvec}^{\prime\prime} \mathcal{B}_{k,\lvec}^{\prime\prime *} \phi_{\pvec}^{\prime\prime *}
...
\begin{eqn}
	={} & 2 \sum_{j=1}^C \Real \left(
		...
		\right) \mathcal{F} \upd t
	+ \sum_{j=1}^C \sum_{k=1}^C \sum_{\lvec \in \mathbb{L}}
			\int \upd\xvec^\prime \mathcal{B}_{j,\lvec}^\prime
			\mathcal{B}_{k,\lvec}^{\prime *}
			\frac{\fdelta}{\fdelta \Psi_j^\prime}
			\frac{\fdelta}{\fdelta \Psi_k^{\prime *}} \mathcal{F} \upd t \\
	& + 2 \sum_{j=1}^C \sum_{\lvec \in \mathbb{L}} \Real \left(
			\int \upd\xvec^\prime \mathcal{B}_{j,\lvec}^\prime
			\upd Q_{\lvec}^\prime
			\frac{\fdelta}{\fdelta \Psi_j^\prime}
		\right) \mathcal{F},
...
Alternatively, the functional It\^{o} formula can be written in matrix form as
\begin{eqn}
	\upd \mathcal{F}[\Psivec]
	={} & \int \upd\xvec^\prime \left(
		2 \Real \left(
			\mathbfcal{A}^\prime \cdot \vfdelta_{\bPsi^\prime}
		\right) \mathcal{F}[\Psivec] \upd t
		+ \Trace{
			\mathcal{B}^\prime
			(\mathcal{B}^\prime)^H
			\vfdelta_{\Psivec^{\prime *}}
			\vfdelta_{\Psivec^\prime}^T
		} \mathcal{F}[\Psivec] \upd t \right. \\
	& \left. + 2 \Real \Trace{
			\mathcal{B}^\prime
			\upd\mathbf{Q}^\prime
			\vfdelta_{\Psivec^\prime}^T
