...
\label{eqn:bec-noise:visibility:correction}
    \mathcal{V}_{\mathrm{noisy}}(t)
    = \mathcal{V}(t) \exp \left(
        -\frac{\sigma_{\mathrm{img}}(t)^2 + \sigma_{\mathrm{MW}}^2}{2} \right),
\end{eqn}
...
Same as in the case of the regular Ramsey sequence, the predictions of the simulation can be improved by including the technical noise according to~\eqnref{bec-noise:visibility:correction}.
