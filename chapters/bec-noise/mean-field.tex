...
The state of a classical \abbrev{bec} is described by wavefunctions $\Psi_j$, which are normalized such that $\int |\Psi_j|^2 \upd\xvec \equiv N_j$, where $N_j$ is the total population of the component $j$ (consequently, $|\Psi_j|^2 \equiv n_j$ is the component density).
...
\end{eqn}
These equations were obtained empirically, but we will see later in this chapter how the same (in the first-order term) expressions appear as a result of the application of the truncated Wigner method to an exact operator equation.

The difference between the internal energies of components $\ket{1}$ and $\ket{2}$ is the hyperfine frequency $\omega_{hf} = \omega_1^{(i)} - \omega_2^{(i)}$.
...
At the beginning of the simulation, a \abbrev{bec} is assumed to be in the ground state which has the lowest possible energy.
...
The most common method of finding the ground state is the imaginary time propagation~\cite{Chiofalo2000,Bao2004}.
...
After the substitution $t \rightarrow \tau = it$ the solution becomes fading, with higher-energy components fading faster:
\begin{eqn}
...
Two-component ground state for \textbf{(a)}~a miscible and \textbf{(b)}~an immiscible regime of \Rb{} \abbrev{bec} with $N_1 = N_2 = 40,000$ atoms in a three-dimensional harmonic trap with the frequencies $f_x = f_y = 97.6\un{Hz},\,f_z = 11.96\un{Hz}$.
...
\end{eqn}
They have to be much smaller than the corresponding radii, which gives us
\begin{eqn}
...
\caption[Numerically calculated and Thomas-Fermi approximated ground states]{Numerically calculated (blue solid lines) and Thomas-Fermi approximated (red dashed lines) ground states for a one-component \Rb{} \abbrev{bec} of \textbf{(a)}~$100,000$ atoms, and \textbf{(b)}~$1,000$ atoms.}%endcaption
...
However, for a lower amount of atoms, say $N_1=10^3$, we get $2 \mu / (\hbar |\bomega|) \approx 1.69$, which is the sign that we are reaching the limit of the approximation's applicability.
