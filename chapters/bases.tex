...
The numerical algorithm for the decomposition of a funtion defined on a discrete grid in the harmonic basis~\cite{Dion2003} is much less known than the \abbrev{fft}, and has some unusual properties, so it is worth describing in brief.
...
Changing variables in the integral to make it comply to the form~\eqnref{bases:gh-quadrature} and applying the quadrature:
\begin{eqn}
    \alpha_m
...
First, beware of significant roundoff errors during the expansion for the large $N$ due to the exponential term in the expression for weights~\eqnref{bases:gh-weights}.
