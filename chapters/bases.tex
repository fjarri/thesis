...
The method is based on the Gauss-Hermite quadrature~\cite{Abramowitz1972}, which states that the value of an integral can be approximated as a sum
\begin{eqn}
...
Changing variable in the integral to make it comply to the form~\eqnref{bases:gh-quadrature} and applying the quadrature:
\begin{eqn}
    \alpha_m
...
First, beware of significant roundoff errors during the expansion for large $N$ due to the exponential term in the expression for weights~\eqnref{bases:gh-weights}.
...
This is a usual case when \abbrev{dht} is used to solve a nonlinear Schr\"odinger equation in momentum space.
